...
void showIngredientDetail(
  ...
  FridgeItem item,
  void Function(FridgeItem) onSave,
) {
...
class _IngredientDetailSheetState extends State<IngredientDetailSheet> {
  ...
  void initState() {
    ...
    _quantity =
        int.tryParse(
          widget.item.quantity?.replaceAll(RegExp(r'[^0-9]'), '') ?? '1',
        ) ??
        1;
    _reminder = widget.item.expiryReminder;
    if (widget.item.daysUntilExpiry != null) {
      _expiryDate = DateTime.now().add(
        Duration(days: widget.item.daysUntilExpiry!),
      );
    }
  }
                  ...
                  Row(
                    children: [
                      _StorageCard(
                        ...
                        onTap: () => setState(
                          () => _storageType = StorageType.refrigerated,
                        ),
                      ),
                      ...
                      _StorageCard(
                        ...
                        onTap: () =>
                            setState(() => _storageType = StorageType.roomTemp),
                      ),
                      const SizedBox(width: 10),
                      _StorageCard(
                        emoji: '🧊',
                        ...
                        onTap: () =>
                            setState(() => _storageType = StorageType.frozen),
                      ),
                    ],
                  ),
